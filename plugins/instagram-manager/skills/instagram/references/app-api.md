...
|---|---|
| The publishing queue and scheduling | Instagram has no scheduling API; the queue is local |
...
```
GET    /api/ig/schedule            list queue + counts by status
POST   /api/ig/schedule            add {kind, caption, imageUrl|imageFile, publishAt}
PATCH  /api/ig/schedule            {id, publishAt?, approved?}
DELETE /api/ig/schedule?id=        remove one
GET    /api/ig/schedule/run        what is due, plus scheduler health
POST   /api/ig/schedule/run        publish everything due now
```

`publishAt` is ISO 8601; `null` keeps it a draft. Whether approval is required
depends on the `autoPublish` setting — read `GET /api/ig/schedule/run` to see
which mode is active before telling the user their post will go out.
...
GET /api/settings          brand voice, avoid list, topics, cadence target,
                           escalation keywords, autoPublish
