...
Both surfaces depend on two providers, and the commonest confusion is not
knowing which one is missing. Put this strip at the top of every artifact:

| Provider | Powers | Shown as |
|---|---|---|
| Composio | YouTube Data API calls — uploads, metadata, playlists, comments, captions | ● Connected / ○ Not connected |
| Zernio | Scheduling, queue slots, comment-to-DM, cross-post, measured analytics | ● Connected / ○ Not connected |
...
|---|---|
| The queue, what is going out, scheduling | Grouped list: **Scheduled** / **Drafts** / **Done**, each row with local time, relative time and a state pill. State that a scheduled upload goes up **private now** and flips public at its slot, and that quota is spent at upload. |
| A report, insights, best time | Stat row, then the detail. Retention curves need their ~3-day finalisation lag marked (`provisionalSince`), and `audienceWatchRatio` above 1 is normal — rewinds and looping Shorts. Convert best-time slots from UTC to the viewer's timezone. |
| Drafting a title or description | The composer: title counter against 100 chars, description against 5,000, tags against 500 combined. Show the title separately — it is not the first line of the description. |
| Comments or DMs to answer | Triage list with the drafted reply editable inline, and the original visible beside it. |
| Cross-posting | One column per target with its own text and counter, and the media requirement per platform stated. |
...
"Upload Queue", "Retention Curve", "Comment Triage". Not "Report" and not a
