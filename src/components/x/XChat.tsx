...
const SUGGESTIONS = [
  "What's my follower count and monthly post usage?",
  "Search for mentions of my handle in the last week",
  "Show my most recent posts with their metrics",
  "What lists do I own?",
];
    ...
    <div className="flex min-h-[60vh] flex-col">
        ...
        {messages.length === 0 && (
          <div className="space-y-4">
            <p className="text-sm text-black/55">
              The AI can use all 79 X tools. It asks before anything that posts,
              sends, follows or deletes.
            </p>
            <div className="grid gap-2 sm:grid-cols-2">
              {SUGGESTIONS.map((s) => (
                <button
                  key={s}
                  onClick={() => send(s)}
                  className="rounded-xl border border-black/10 bg-white px-4 py-3 text-left text-sm transition hover:border-brand/40"
                >
                  {s}
                </button>
              ))}
      ...
      >
        <input
          ...
          placeholder="Ask anything about your X account…"
          disabled={busy}
